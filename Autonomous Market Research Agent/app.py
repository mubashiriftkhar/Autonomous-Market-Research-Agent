...
app = FastAPI(
    title="Autonomous Market Research Agent API",
    description="API for processing market research queries",
    version="1.0.0"
)
...
async def process_research_query(request: QueryRequest):
    ...
    "summarizedResult": "",
        ...
        html_response = graph.invoke(input=input_state)
    # html_response = await generate_html_response(request.query)
    # return {"html": html_response}
        return {"html": html_response["summarizedResult"]}
